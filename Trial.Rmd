...
```{r}
str(df1$FlightDate)
df <- as.Date(df1$FlightDate, format("%m/%d/%y"))
df
head(df1$FlightDate)
df<- substr(df1$FlightDate, nchar(df1$FlightDate)-8, nchar(df1$FlightDate))
df<- substr(df1$FlightDate,1, nchar(df1$FlightDate)-4)
df<-substr(df,1,4)
df
df<-as.Date(df, format = "%m/%d/%y" )
df
```
